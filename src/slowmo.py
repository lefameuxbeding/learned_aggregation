...
class SGDSlowMo(OptaxOptimizer):
  ...
  def init(self,
    ...
    return OptaxState(  # pytype: disable=wrong-arg-types  # jax-ndarray
        params=params,
        optax_opt_state= [self.opt.init(params), {'momentum':copy.deepcopy(params)} if momentum is None else {'momentum':momentum},], 
        state=model_state,
        iteration=0,
    )
  ...
  def update(self,
             ...
             **kwargs):
    del loss
    update, new_opt_state = self.opt.update(grad, opt_state.optax_opt_state[0],
                                            opt_state.params)
    return OptaxState(
        state=model_state,
        params=optax.apply_updates(opt_state.params, update),
        optax_opt_state=[new_opt_state,opt_state.optax_opt_state[1],],
