...
@gin.configurable
class MuHyperV2(lopt_base.LearnedOptimizer):
  ...
  def _ff_mod(self, global_feat, extra_step_mult, p, g, m, rms, fac_g,
    ...
    if self.param_scale_mult:
      step = direction * (param_scale * mag_param) * self.step_mult * mup_lr_scale
    else:
      step = direction * mag_param * self.step_mult * mup_lr_scale
  ...
  def init(self, key) -> lopt_base.MetaParams:
    ...
    def ffmod_init(key):
      ...
      mod_theta = self.ff_mod.init(key, global_features, 1.0, p, g, m, rms,
                                   fac_g, fac_vec_col, fac_vec_row, fac_vec_v, mup_lr_scale)
      return mod_theta
  ...
  def opt_fn(self, theta, is_training=True) -> opt_base.Optimizer:
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def update(self,
        ...
        mup_lr_scale = model_state['mup_lrs_to_use']
        ...
        next_params = jax.tree_util.tree_map(
            ...
            next_adafac_rolling.v_diag, mup_lr_scale)
